...
if(ismember == null)	ismember = "";	// 비회원 구매하기를 눌러 로그인 페이지에 들어왔는지 여부를 담는 변수 
...
if (request.getParameter("optCnt") != null) {	// 옵션이 있으면
	...
	optValue = optValue.substring(1);	// 선택한 옵션들을 쉼표를 구분자로 하여 문자열로 만듦
}
%>
...
<script>
function goJoin() {// 회원 가입 폼으로 이동시키는 함수
...
function goDirect() { // 로그인 없이 비회원인 상태로 구매할 때 구매폼으로 이동시키는 함수
	var frm = document.frmLogin;
	document.getElementById("ismember").value = "n";
