...
<tr class="nvcon">
<td align="center" width="10%">제목</td><td width="30%"><%=article.getFq_title() %></td>
<td width="*" colspan="2"><td>
</tr>
<tr class="nvcon">
<td align="center">작성자</td><td width="10"><%=article.getFq_writer() %></td>
<td align="center" width="10%">작성일</td><td><%=article.getFq_date().substring(2, 10).replace('-', '.') %></td>
<td width="*" colspan="2"></td>
</tr>
<tr>
<td colspan="6">
<textarea id="content" name="content" rows="10" cols="60">
<%=article.getFq_content().replace("\r\n", "<br />") %></textarea>
</td></tr>
<%
boolean isPms = false;	// 수정 및 삭제 권한이 있는지 여부를 저장할 변수
String link1 = null, link2 = null;

if (uid != null && uid.equals(article.getFq_writer())) {
	isPms = true;
	link1 = "location.href='bbs_form.afaq?wtype=up&idx=" + idx + "';";
	link2 = "notCool(" + idx + ");";
} 
...
function notCool(idx) {
	if (confirm("정말 삭제하시겠습니까?")) {
		location.href="bbs_proc.afaq?wtype=del&idx=" + idx;
	}
}
</script>
<tr>
