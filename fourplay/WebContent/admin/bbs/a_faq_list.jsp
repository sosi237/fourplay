...
if (articleList != null && rcnt > 0) {
	...
	for (int i = 0 ; i < articleList.size() ; i++) {
		title = articleList.get(i).getFq_title();
		lnk = "<a href='bbs_view.afaq?idx=" + 
			articleList.get(i).getFq_idx() + args + 
			"' title='" + title + "'>";
...
<td><%=articleList.get(i).getFq_writer() %></td>
<td><%=articleList.get(i).getFq_date().substring(2, 10).replace('-', '.') %></td>
</tr>
<%
	}
} else {	// 검색결과가 없으면
...
if (rcnt > 0) {
	...
	if (cpage == 1) {
		...
		out.print("<a href='bbs_list.afaq?cpage=1" + schargs + "'>");
		out.println("[<<]</a>&nbsp;&nbsp;");
		out.print("<a href='bbs_list.afaq?cpage=" + (cpage - 1) + schargs + "'>");
		out.println("[<]</a>&nbsp;&nbsp;");
	}
	...
	for (int i = 1, j = spage ; i <= 10 && j <= pcnt ; i++, j++) {
		if (cpage == j) {
			...
			out.print("&nbsp;<a href='bbs_list.afaq?cpage=" + j + schargs + "'>");
			out.println(j + "</a>&nbsp;");
		}
	}
	...
	if (cpage == pcnt) {
		...
		out.print("&nbsp;&nbsp;<a href='bbs_list.afaq?cpage=" + (cpage + 1) + schargs + "'>");
		out.println("[>]</a>");
		out.print("&nbsp;&nbsp;<a href='bbs_list.afaq?cpage=" + pcnt + schargs + "'>");
		out.println("[>>]</a>");
	}
}
%>
...
<td width="10%"><input id="submit" type="button" value="글 등록" onclick="location.href='bbs_form.afaq?wtype=in';" />
